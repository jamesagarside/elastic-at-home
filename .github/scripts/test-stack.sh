...
log_debug()   { [[ "$DEBUG" == "true" ]] && echo -e "${GRAY}[DEBUG]${NC} $1" >&2 || true; }
...
_curl_opts() {
    ...
    if [[ "$INGRESS_MODE" == "letsencrypt" ]]; then
        # LE staging certs are not in the system trust store
        opts="$opts -k"
    elif [[ -f "$CA_CERT" ]]; then
        opts="$opts --cacert $CA_CERT"
    fi
    echo "$opts"
}
...
test_ca_certificate() {
    ...
    if [[ "$INGRESS_MODE" == "letsencrypt" ]]; then
        # Verify the cert was actually issued by LE (not a Traefik self-signed fallback)
        local es_host
        es_host=$(echo "$ELASTICSEARCH_URL" | sed -E 's|https?://([^:/]+).*|\1|')
        local issuer
        issuer=$(echo | openssl s_client -connect "${es_host}:443" -servername "$es_host" 2>/dev/null \
            | openssl x509 -noout -issuer 2>/dev/null || echo "")

        if echo "$issuer" | grep -qiE "(let's encrypt|letsencrypt|staging|artificial)"; then
            record_pass "Let's Encrypt certificate verified (issuer: $issuer)"
        elif [[ -z "$issuer" ]]; then
            record_fail "Could not retrieve certificate from ${es_host}:443"
            return 1
        else
            record_fail "Certificate not from Let's Encrypt (issuer: $issuer) - possible Traefik self-signed fallback"
            return 1
        fi
        return 0
    fi
...
test_agent_enrolled() {
    log_test_start "Elastic Agent Enrolment"
    ...
    if [[ "$status" == "200" ]]; then
        ...
        record_pass "Fleet API accessible (agent enrolment may be pending)"
        return 0
    fi
